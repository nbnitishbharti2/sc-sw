				...
				<div class="container-fluid">
					<div class="row mb-2">
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 right-align">
							...
							<a href="{!! route('add.roles') !!}" class="btn btn-primary"><i class="fa fa-plus"></i>  &nbsp; {{ trans('button.add') }}</a>
						</div>
					</div>
				</div><!-- /.container-fluid -->
			</section>
														...
														<td>
															@if(ucfirst($item->name) != 'Superadmin')
																@if(empty($item->deleted_at))
																	<a href="{!! route('edit.roles', $item->id) !!}"  title="{{ trans('button.edit') }}" class="btn-sm btn btn-primary"><i class="fa fa-edit"></i>  </a>
																	<button class="btn-sm btn btn-danger" title="{{ trans('button.delete') }}" onclick="confirm_delete({{ $item->id }})"><i class="fa fa-trash"></i></button>
																@else
																	<button class="btn-sm btn btn-success" title="{{ trans('button.restore') }}" onclick="confirm_restore({{ $item->id }})"><i class="fa fa-undo"></i></button>
																@endif
															@endif
														</td>
													</tr>
