...
<body class="hold-transition sidebar-mini layout-fixed">
				...
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<h1>{{Lang::get('title.view_vehicle_type')}}</h1>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 right-align">
							...
							<a href="{!! route('add.vehicle_type') !!}" class="btn btn-primary"><i class="fa fa-plus"></i>  &nbsp; {{Lang::get('button.add')}}</a>
						</div>
					</div>
				</div><!-- /.container-fluid -->
			</section>
										...
										<table id="table" class="table table-bordered table-striped">
											<thead>
												<tr>
													...
													<th>{{Lang::get('title.vehicle_type_name')}}</th>
	    ...
	    <div class="modal-dialog">
	      <div class="modal-content">
	        <div class="modal-header">
	          <h4 class="modal-title">{{Lang::get('title.delete_vehicle_type')}}</h4>
	          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	            <span aria-hidden="true">&times;</span>
	          </button>
	        </div>
	        <div class="modal-body">
	          <p> {{Lang::get('message.delete_vehicle_type')}}</p>
	        </div>
	        <div class="modal-footer">
	          <button type="button" class="btn btn-danger" data-dismiss="modal">{{Lang::get('button.close')}}</button>
	          <a href="{{ url('/delete-vehicle-type/11') }}" id="delete-vehicle_type" class="btn btn-primary">{{Lang::get('button.delete')}}</a>
	        </div>
	      </div>
	      <!-- /.modal-content -->
	    </div>
	...
	<div class="modal fade" id="modal-vehicle_type-restore">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">{{Lang::get('title.retore_vehicle_type')}}</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <p> {{Lang::get('message.restore_vehicle_type')}}</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-danger" data-dismiss="modal">{{Lang::get('button.close')}}</button>
              <a href="{{ url('/restore-vehicle-type/11') }}" id="restore-vehicle_type" class="btn btn-primary">{{Lang::get('button.restore')}}</a>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
