...
class FeeRepository {
    ...
    */
    ...
    {
    	try {
            return  $query = Fee::where('session_id',Session::get('session'))->with(['fee_head','fee_type','fee_frequency']) ->withTrashed()->get(); 
        } catch(\Exception $err){
    ...
    public function edit($fee_id)
    {
        try {
            ...
            $data = [
                ...
                'frequency_id'    => (old('frequency_id')) ? old('frequency_id') : $fee->fee_frequency->id,
                'frequency_value' => $fee->fee_frequency->value,
                'month_list'      => Month::getAllMonthForListing(),
                'month_ids'       => $applied_in_month_ids,   
            ];
            return $data;
    ...
    */
    public function update($request)
    {
        try {
            ...
            if($fee->wasChanged() || $removed_month_ids || $fee_months) { //Check if data was updated
                return true;
            } else {
                return false;
            }
